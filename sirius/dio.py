...
def get_available_ant_models():
    ...
    # Little function that finds the first number in a string (used for sorting names).
    ...
    telescope_layout_files = [os.path.basename(x) for x in tel_paths]

    supported_telescopes = {"alma": [], "aca": [], "evla": [], "vla": [], "ngvla": [], "meerkat": []}

    # Here we organize by telescope
    for tlf in telescope_layout_files:
        for tel_name in supported_telescopes.keys():
            if tel_name == "vla":
                if (tel_name in tlf.lower()) and ("ngvla" not in tlf.lower()) and ("evla" not in tlf.lower()):
                    supported_telescopes[tel_name].append(tlf)
            else:
                if tel_name in tlf.lower():
                    supported_telescopes[tel_name].append(tlf)

    for key in supported_telescopes:
        supported_telescopes[key].sort(key=num_sort)
    ...
    apc_files = [os.path.basename(x) for x in apc_paths]

    supported_apc = {"evla": [], "meerkat": []}

    # Here we organize by telescope
    for apc_file in apc_files:
        for apc_name in supported_apc.keys():
            if apc_name in apc_file.lower():
                supported_apc[apc_name].append(apc_file)

    for key in supported_apc:
        supported_apc[key].sort(key=num_sort)
    print_dict_as_html_table(supported_apc, "Available Zernike Polynomial Coefficient Models")

    ####### Available Beam Polynomial Coefficient Models ########
    bpc_dir = pkg_resources.resource_filename("sirius_data", "beam_polynomial_coefficient_models/data/")
    bpc_paths = glob.glob(bpc_dir + "/*.bpc.zarr")
    bpc_files = [os.path.basename(x) for x in bpc_paths]

    supported_bpc = {"evla": []}

    # Here we organize by telescope
    for bpc_file in bpc_files:
        for bpc_name in supported_bpc.keys():
            if bpc_name in bpc_file.lower():
                supported_bpc[bpc_name].append(bpc_file)

    for key in supported_bpc:
        supported_bpc[key].sort(key=num_sort)
...
def create_mxds(
    ...
    chunks = {
        "row": (xds0["DATA"].chunks[0][0],),
        "chan": (xds0["DATA"].chunks[1][0],),
        "pol": (xds0["DATA"].chunks[2][0],),
        "uvw": (3,),
    }
    ...
    exposures = da.full(
        (n_row,), time_xda.time_delta, chunks=chunks[0], dtype="float64"
    )  # fill with input in units of the input array, which we
...
def write_zarr(
    ...
    xr.Dataset.to_zarr(
        ddi_indx_xds, store=outfile + "/DDI_INDEX", mode="w", encoding=encoding, consolidated=consolidated
    )
...
def _fix_dict_for_ms(name, xds):
    ...
    if "xds" in name:
        xds.column_descriptions["UVW"]["shape"] = np.array(xds.column_descriptions["UVW"]["shape"].split(",")).astype(
            int
        )

    if "SPECTRAL_WINDOW" == name:
        # print('2.',xds.column_descriptions)
        xds.column_descriptions["CHAN_FREQ"]["keywords"]["MEASINFO"]["TabRefCodes"] = np.array(
            xds.column_descriptions["CHAN_FREQ"]["keywords"]["MEASINFO"]["TabRefCodes"].split(",")
        ).astype(int)
        xds.column_descriptions["REF_FREQUENCY"]["keywords"]["MEASINFO"]["TabRefCodes"] = np.array(
            xds.column_descriptions["REF_FREQUENCY"]["keywords"]["MEASINFO"]["TabRefCodes"].split(",")
        ).astype(int)
...
def _fix_dict_for_zarr(name, xds):
    ...
    if "SPECTRAL_WINDOW" == name:
        ...
        xds.column_descriptions[0]["CHAN_FREQ"]["keywords"]["MEASINFO"]["TabRefCodes"] = ",".join(
            map(str, xds.column_descriptions[0]["CHAN_FREQ"]["keywords"]["MEASINFO"]["TabRefCodes"])
        )
        xds.column_descriptions[0]["REF_FREQUENCY"]["keywords"]["MEASINFO"]["TabRefCodes"] = ",".join(
            map(str, xds.column_descriptions[0]["REF_FREQUENCY"]["keywords"]["MEASINFO"]["TabRefCodes"])
        )

    if "ANTENNA" == name:
        xds.column_descriptions[0]["OFFSET"]["shape"] = ",".join(
            map(str, xds.column_descriptions[0]["OFFSET"]["shape"])
        )
        xds.column_descriptions[0]["POSITION"]["shape"] = ",".join(
            map(str, xds.column_descriptions[0]["POSITION"]["shape"])
        )
    ...
    if "OBSERVATION" == name:
        xds.column_descriptions[0]["TIME_RANGE"]["shape"] = ",".join(
            map(str, xds.column_descriptions[0]["TIME_RANGE"]["shape"])
        )
