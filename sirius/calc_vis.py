...
c = 299792458
from ._sirius_utils._direction_rotate import (
    _calc_rotation_mats,
    _cs_calc_rotation_mats,
    _directional_cosine,
    _sin_project,
)
from ._sirius_utils._apply_primary_beam import (
    _apply_casa_airy_pb,
    _apply_airy_pb,
    apply_casa_airy_pb,
    apply_airy_pb,
...
def calc_vis(
    ...
    (
        ...
        beam_types,
        new_beam_model_map,
    ) = beam_models_to_tuple(
        beam_models, beam_model_map
    )  # Needs to be done for numba
    ...
    calc_vis_jit(
        vis_data,
        uvw,
        tuple(vis_data_shape),
        point_source_flux.astype(np.complex128),
        point_source_ra_dec,
        pointing_ra_dec,
        phase_center_ra_dec,
        antenna1,
...
def calc_vis_jit(
    ...
    for i_time in range(n_time):
        ...
        for i_baseline in range(n_baseline):
            ...
            if do_pointing:
                ra_dec_in_1 = pointing_ra_dec[
                    i_time // f_pt_time, i_ant_1 // f_pt_ant, :
                ]
                ra_dec_in_2 = pointing_ra_dec[
                    i_time // f_pt_time, i_ant_2 // f_pt_ant, :
                ]
            ...
            for i_point_source in range(n_point_source):
                ...
                if not (
                    np.array_equal(prev_ra_dec_in, ra_dec_in)
                    and np.array_equal(prev_ra_dec_out, ra_dec_out)
                ):
                ...
                if do_pointing:
                    if not (
                        np.array_equal(prev_ra_dec_in, ra_dec_in_1)
                        and np.array_equal(prev_ra_dec_out, ra_dec_out)
                    ):
                        lm_sin_1 = _sin_project(
                            ra_dec_in_1, ra_dec_out
                        )  # NBNBNB might need to change to _calc_rotation_mats depending
                        sep_1 = 2.0 * np.arcsin(np.sqrt(np.sum(lm_sin_1 ** 2)) / 2.0)
                    if not (
                        np.array_equal(prev_ra_dec_in, ra_dec_in_2)
                        and np.array_equal(prev_ra_dec_out, ra_dec_out)
                    ):
                ...
                for i_chan in range(n_chan):
                    ...
                    # s1 = time.time()
                    flux = point_source_flux[
                        i_point_source, i_time // f_sf_time, i_chan // f_sf_chan, :
                    ]
                    ...
                    # s2 = time.time()
                    if do_pointing:
                        flux_scaled, outside_beam = calc_pb_scale(
                            flux,
                            sep_1,
                            sep_2,
                            bm1_indx,
                    ...
                    if not outside_beam:
                        ...
                        for i_pol in range(n_pol):
                            ...
                            vis_data[i_time, i_baseline, i_chan, i_pol] = vis_data[
                                i_time, i_baseline, i_chan, i_pol
                            ] + flux_scaled[pol[i_pol]] * np.exp(phase_scaled) / (
                                1 - lmn_rot[2]
                            )
...
def beam_models_to_tuple(beam_models, beam_model_map):
    ...
    return (
        tuple(beam_models_list_type0),
        tuple(beam_models_list_type1),
        tuple(beam_types),
        tuple(new_beam_model_map),
    )
...
def calc_pb_scale(
    ...
    outside_beam = False
    if (
        (bm1_indx == bm2_indx) and (bm1_type == bm2_type) and ~do_pointing
        ...
        if bm1_type == 0:  # Checks if it is a zernike model
            ...
            if sep1 < max_rad:
                # bm1 = beam_models_type0[bm1_indx]
                J_sampled = sample_J(
                    bm1[1], bm1[2], bm1[3], bm1[4], bm1[5], bm1[6], pa, freq, lmn1
                )[:, 0]
        ...
        if bm2_type == 0:
            ...
            if sep2 < max_rad:
                J_sampled2 = sample_J(
                    bm2[1], bm2[2], bm2[3], bm2[4], bm2[5], bm2[6], pa, freq, lmn2
                )[:, 0]
            else:
                outside_beam = True
        else:
...
def make_mueler_mat(J1, J2, mueller_selection):
    ...
    for m_flat_indx in mueller_selection:
        ...
        M[m_flat_indx // 4, m_flat_indx - 4 * (m_flat_indx // 4)] = J1[
            map_mueler_to_pol[m_flat_indx, 0]
        ] * np.conj(J2[map_mueler_to_pol[m_flat_indx, 1]])
