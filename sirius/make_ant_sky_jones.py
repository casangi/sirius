...
from ._sirius_utils._calc_parallactic_angles import (
    _calc_parallactic_angles,
    _find_optimal_set_angle,
)
import itertools
...
def evaluate_beam_models(
    beam_models, beam_parms, freq_chan, phase_center_ra_dec, time_str, site
):
...
def make_ant_sky_jones(list_zpc_dataset, beam_parms):
    ...
    _beam_parms["needed_pol"] = np.unique(
        np.ravel(map_mueler_to_pol[_beam_parms["mueller_selection"]])
    )

    assert (0 in _beam_parms["mueller_selection"]) or (
        15 in _beam_parms["mueller_selection"]
    ), "Mueller element 0 or 15 must be selected."
...
def _calc_resolution(pb_freq, list_zpc_xds, beam_parms):
    ...
    for zpc_xds in list_zpc_xds:
        ...
        delta = min(
            min(fov / beam_parms["image_size"][0]),
            min(fov / beam_parms["image_size"][1]),
        )
        if delta < min_delta:
            min_delta = delta
    return min_delta
