...
def _calc_ant_jones(list_zpc_dataset, j_freq, j_pa, beam_parms):
    ...
    j_planes = np.zeros(
        (
            len(list_zpc_dataset),
            len(j_pa),
            len(j_freq),
            len(beam_parms["needed_pol"]),
            beam_parms["image_size"][0],
            beam_parms["image_size"][1],
        ),
        np.complex128,
    )
    j_planes_shape = j_planes.shape
    iter_dims_indx = itertools.product(
        np.arange(j_planes_shape[0]),
        np.arange(j_planes_shape[1]),
        np.arange(j_planes_shape[2]),
    )
    ...
    for i_model, i_pa, i_chan in iter_dims_indx:
        ...
        if (i_model != i_model_prev) or (freq != freq_prev):
            beam_interp = beam.interp(
                chan=freq, method=beam_parms["zernike_freq_interp"]
            )
        ...
        eta = beam_interp.ETA[
            0, 0
        ].values  # Assume ETA is the same for all pol and coef. Only a function of freq. Need to update zpc.zarr format.
        ...
        for i_pol, pol in enumerate(beam_parms["needed_pol"]):
            a = _generate_zernike_surface(
                beam_interp.ZC.data[pol, :].compute(), x_grid, y_grid
            )
            a[r_grid > 1] = 0
            j_planes[
                i_model,
                ...
                i_chan,
                i_pol,
                ic[0] - ic_z[0] : ic[0] + ic_z[0] + include_last[0],
                ic[1] - ic_z[1] : ic[1] + ic_z[1] + include_last[1],
            ] = a
            j_planes[i_model, i_pa, i_chan, i_pol, :, :] = np.fft.fftshift(
                np.fft.ifft2(
                    np.fft.ifftshift(j_planes[i_model, i_pa, i_chan, i_pol, :, :])
                )
            ) / (beam_parms["image_size"][0] * beam_parms["image_size"][1])
        ...
        if 3 not in beam_parms["needed_pol"]:
            P_max = np.abs(
                j_planes[
                    ...
                    i_pa,
                    i_chan,
                    np.where(beam_parms["needed_pol"] == 0),
                    j_planes_shape[4] // 2,
                    j_planes_shape[5] // 2,
                ]
            )
            ...
            Q_max = np.abs(
                j_planes[
                    ...
                    i_pa,
                    i_chan,
                    np.where(beam_parms["needed_pol"] == 3),
                    j_planes_shape[4] // 2,
                    j_planes_shape[5] // 2,
                ]
            )
        ...
        j_planes[i_model, i_pa, i_chan, :, :, :] = (
            j_planes[i_model, i_pa, i_chan, :, :, :] * 2 / (P_max + Q_max)
        )
