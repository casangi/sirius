...
@jit(
    numba.float64[::1](types.Array(types.float64, 2, "A", readonly=True), numba.float64[::1], numba.float64[::1]),
    nopython=True,
    cache=True,
    nogil=True,
)
...
# @jit(types.Array(types.complex128, 2, 'A')(types.Array(types.complex128, 3,
#'C'), types.Array(types.float64, 1, 'C'), types.Array(types.float64, 1, 'C'),
# types.float64, types.float64), nopython=True,cache=False,nogil=True)
# @jit(types.Array(types.complex128, 2)(types.Array(types.complex128, 3),
# types.Array(types.float64, 1), types.Array(types.float64, 1), types.float64,
# types.float64), nopython=True,cache=True,nogil=True)
@jit(
    types.Array(types.complex128, 2, "A")(
        types.Array(types.complex128, 3, "A"),
...
def _interp_array(im_array, l_ind, m_ind, delta_l, delta_m):
    """Interpolates image values at coordinates (l_ind, m_ind).
    Inputs
    --------------
    im_array: 3-d numpy array of shape (pol, image, image)
    l_ind: 1-d numpy array of fractional indices (float)
    m_ind: 1-d numpy array of fractional indices (float)
    delta_l: pixel size for l coordinates (float)
    delta_m: pixel size for m coordinates (float)
    Notes: l_ind and m_ind must be same length.
    --------------
    Outputs:
    --------------
    2-d numpy array of interpolated values (float) with shape (pol,len(l_ind))
    """
    ...
    x_frac = (l_ind / delta_l) + n_l // 2
    # Fractional pixel along m
    y_frac = (m_ind / delta_m) + n_m // 2
    # Numba-style array creation. Shape is (polarization, coordinates)
    results = np.zeros((len(im_array), len(l_ind)), dtype=numba.complex128)
    for i in range(len(im_array)):
        # Complex interpolation
        results[i] = _bilinear_interpolate(im_array[i].real, x_frac, y_frac) + 1j * _bilinear_interpolate(
            im_array[i].imag, x_frac, y_frac
        )
...
# @jit(types.Array(types.float64, 2, 'C')(types.Array(types.float64, 2, 'C'),
# numba.int64), nopython=True, cache=True, nogil=True)
def _powl(base, exp):
    # print(base,exp)
    # base can be any real and exp must be positive integer
    """
