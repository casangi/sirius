...
def _sin_project(ra_dec_o, ra_dec):
    ...
    lm[1] = np.sin(dec) * np.cos(dec_o) - np.cos(dec) * np.sin(dec_o) * np.cos(
        ra - ra_o
    )
    return lm
...
def _calc_rotation_mats(ra_dec_in, ra_dec_out):
    ...
    uvw_rotmat = (
        _func_R_x(-(np.pi / 2 - ra_dec_in[1]))
        @ _func_R_z(ra_dec_out[0] - ra_dec_in[0])
        @ _func_R_x(np.pi / 2 - ra_dec_out[1])
    )
    ...
    lmn_rot = out_rotmat @ (
        lmn_out - lmn_in
    )  # out_rotmat(lmn_out - lmn_in)= [0,0,1] - out_rotmat@[l_in,m_in,n_in]
...
def _cs_calc_rotation_mats(ra_dec_in, ra_dec_out, rotation_parms):
    ...
    out_rotmat = R.from_euler(
        "XZ", [[-ra_dec_out[1] + np.pi / 2, -ra_dec_out[0] + np.pi / 2]]
    ).as_matrix()[0]
    ...
    # x-axis (lat-90).
    in_rotmat = R.from_euler(
        "ZX", [[ra_dec_in[0] - np.pi / 2, ra_dec_in[1] - np.pi / 2]]
    ).as_matrix()[0]
    ...
    R_zx_si = _func_R_z(ra_dec_in[0] - np.pi / 2) @ _func_R_x(
        ra_dec_in[1] - np.pi / 2
    )  # in_rotmat
    R_xz_is = _func_R_x(-ra_dec_in[1] + np.pi / 2) @ _func_R_z(
        -ra_dec_in[0] + np.pi / 2
    )  # R_xz_is = R_zx_si.T
    print("@@@@@@@@@@@in_rotmat", mat_dis(R_zx_si, in_rotmat))

    R_xz_os = _func_R_x(-ra_dec_out[1] + np.pi / 2) @ _func_R_z(
        -ra_dec_out[0] + np.pi / 2
    )  # out_rotmat
    R_zx_so = _func_R_z(ra_dec_out[0] - np.pi / 2) @ _func_R_x(
        ra_dec_out[1] - np.pi / 2
    )
    ...
    if rotation_parms["reproject"] == True:
        ...
        temp = R.from_euler(
            ...
            [
                [
                    -np.pi / 2 + ra_dec_out[1],
                    ra_dec_out[0] - ra_dec_in[0],
                    np.pi / 2 - ra_dec_in[1],
                ]
            ],
        ).as_matrix()[0]
    ...
    if rotation_parms["common_tangent_reprojection"] == True:
        uvw_rotmat[
            2, 0:2
        ] = 0.0  # (Common tangent rotation needed for joint mosaics, see last part of FTMachine::girarUVW in CASA)
