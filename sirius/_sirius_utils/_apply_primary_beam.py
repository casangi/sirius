...
def _apply_casa_airy_pb(lmn, freq_chan, dish_diameter, blockage_diameter, ipower):
    ...
    if (lmn[0] != 0) and (lmn[1] != 0):
        ...
        r = (
            2.0 * np.arcsin(np.sqrt(np.sum(lmn ** 2)) / 2.0) * k * aperture
        )  # use lmn_rot in calc vis
        # print('r',r)
        ...
        if blockage_diameter == 0.0:
            ...
            return (
                (
                    area_ratio * 2.0 * j1(r) / r
                    - 2.0 * j1(r * length_ratio) / (r * length_ratio)
                )
                / (area_ratio - 1.0)
            ) ** ipower
    else:
...
def _apply_airy_pb(lmn, freq_chan, dish_diameter, blockage_diameter, ipower):
    ...
    if (lmn[0] != 0) and (lmn[1] != 0):
        ...
        if blockage_diameter == 0.0:
            ...
            return (
                (2.0 * j1(r) / r - 2.0 * e * j1(r * e) / r) / (1.0 - e ** 2)
            ) ** ipower
            # Changed r_grid to r ^^^
    else:
        return 1
...
def apply_casa_airy_pb(lmn, freq_chan, pb_parms):
    ...
    if (lmn[0] != 0) and (lmn[1] != 0):
        ...
        if blockage_diameter == 0.0:
            ...
            return (
                (
                    area_ratio * 2.0 * jn(1, r) / r
                    - 2.0 * jn(1, r * length_ratio) / (r * length_ratio)
                )
                / (area_ratio - 1.0)
            ) ** ipower
    else:
...
def apply_airy_pb(lmn, freq_chan, pb_parms):
    ...
    if (lmn[0] != 0) and (lmn[1] != 0):
        ...
        if blockage_diameter == 0.0:
            ...
            e = blockage_diameter / dish_diameter
            return (
                (2.0 * jn(1, r) / r - 2.0 * e * jn(1, r * e) / r) / (1.0 - e ** 2)
            ) ** ipower
    else:
        return 1
