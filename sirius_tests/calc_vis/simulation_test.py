...
from sirius._sirius_utils._calc_parallactic_angles import (
...
def test_simulation():
    ...
    point_source_skycoord = SkyCoord(
        ra="19h59m28.5s", dec="+40d44m01.5s", frame="fk5"
    )  # sim
    ...
    assert (np.allclose(vis_data, vis_data_import), np.allclose(uvw, uvw_import)) == (
        True,
        True,
    )
